<script lang="ts">
	...
	let anyLabelActive = $derived(labels.length > 0 && labels.some(l => activeLabels[l] ?? true));
	...
	let visibleIssues = $derived.by(() => {
		...
		if (anyLabelActive) {
			result = result.filter(i => {
				const issueLabels = i.fields.labels ?? [];
				if (issueLabels.length === 0) return true;
				return issueLabels.some(l => activeLabels[l] ?? true);
			});
		}
