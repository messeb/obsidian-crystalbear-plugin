...
import { isMisconfigured, classifyError } from '../error-utils';
...
async function renderJiraBlock(
	...
	if (isMisconfigured(settings)) {
		renderBlockError(el, t.errNotConfigured, t.errNotConfiguredDesc);
		return;
	}
	...
	try {
		...
		wrapper.empty();
		const issueKey = params.type === 'issue' ? params.key : undefined;
		const classified = classifyError(err, issueKey);
		const block = wrapper.createDiv({ cls: 'jira-error-state' });
		block.createEl('span', { text: classified.title, cls: 'jira-error-state-title' });
		block.createEl('span', { text: classified.description, cls: 'jira-error-state-desc' });
	}
}

function renderBlockError(el: HTMLElement, title: string, description: string): void {
	const block = el.createDiv({ cls: 'jira-error-state' });
	block.createEl('span', { text: title, cls: 'jira-error-state-title' });
	block.createEl('span', { text: description, cls: 'jira-error-state-desc' });
}
